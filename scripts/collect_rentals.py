...
def _apply_zillow_when_no_direct_link(df: pd.DataFrame) -> pd.DataFrame:
    """Clear placeholders and Zillow map/search rentals URLs from listing_url.

    Do not store Zillow search links as listing_url (not a property page). The web UI
    generates the same scoped search via zillowRentSearchHref when listing_url is empty.
    """
    out = df.copy()
    ...
    for i in out.index:
        ...
        if _is_placeholder_listing_url(u) or _is_zillow_rentals_search_url(u):
            out.loc[i, "listing_url"] = ""
        else:
